...
contract USDSToUSDC is BaseHealthCheck {
    ...
    constructor(address _vault, string memory _name) BaseHealthCheck(USDS, _name) {
    ...
    function _deployFunds(uint256 _amount) internal override {
        uint256 gemAmount = _amount / SCALE;
        if (gemAmount != 0) {
    ...
    function _freeFunds(uint256 _amount) internal override {
        uint256 gemNeeded = _ceilDiv(_amount, SCALE);
        uint256 looseUsdc = ERC20(USDC).balanceOf(address(this));
    ...
    function _totalConvertedInAsset() internal view returns (uint256) {
        ...
        return _scaleUsdcToAsset(balance);
    }
